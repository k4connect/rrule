      ...
      while (true) {
        ...
        if (plb(bysetpos) && plb(timeset)) {
          ...
          for (j = start; j < end; j++) {
            i = dayset[j]
            if (i !== null) {
              date = dateutil.fromOrdinal(ii.yearordinal + i, timezone)
...
RRule.fromString = function(string) {
  ...
  Iterinfo.prototype.rebuild = function (year, month) {
    ...
    if (year !== this.lastyear) {
      ...
      var firstyday = new Date(year, 0, 1, timeZone)

      this.yearordinal = dateutil.toOrdinal(firstyday)
      this.yearweekday = dateutil.getWeekday(firstyday)

      var wday = dateutil.getWeekday(new Date(year, 0, 1, timeZone))
      ...
      if (!plb(rr.options.byweekno)) {
        ...
        if (no1wkst) {
          ...
          var lnumweeks
          if (!contains(rr.options.byweekno, -1)) {
            var lyearweekday = dateutil.getWeekday(new Date(year - 1, 0, 1, timeZone))
    ...
    init: function (method, args) {
      ...
      if (method === 'between') {
        this.maxDate = args.inc
          ? args.before : new Date(args.before.getTime() - 1, timezone)
        this.minDate = args.inc
          ? args.after : new Date(args.after.getTime() + 1, timezone)
      } else if (method === 'before') {
        this.maxDate = args.inc ? args.dt : new Date(args.dt.getTime() - 1, timezone)
      } else if (method === 'after') {
        this.minDate = args.inc ? args.dt : new Date(args.dt.getTime() + 1, timezone)
      }
    },
